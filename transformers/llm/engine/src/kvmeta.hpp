...
struct BatchKVMeta {
    ...
    // sync after add/remove-on-kvmeta
    void sync();

    // regitster on eache backend
    ...
    // reset all kv cache, called when model changed or batch size changed
    void reset() {
        for (auto& callback : mResetCallbacks) {
            if (callback.second) {
                callback.second();
            }
        }
